...
<template>
  <div class="flex flex-col justify-center items-center">
    <lottie-animation
      class="w-96"
      :animation-data="personAnimation"
      :loop="true"
    ></lottie-animation>
    <h1 class="title">{{ title }}</h1>
    <article
      class="text-xl md:text-2xl text-primary w-10/12 text-center mb-8 dark:text-white transition ease-in-out delay-300"
    >
      {{ text }}
    </article>
    <div class="relative w-80 container h-64 flex flex-col">
      <a class="btn flex justify-center items-center z-50 w-full" href="#ux">
        <lottie-animation
          class="h-48 transition ease-in-out hover:opacity-70"
          :key="animationData"
